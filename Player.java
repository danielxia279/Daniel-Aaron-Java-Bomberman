...
public class Player{
    ...
    public void tick(){
        ...
        if(GameState.grid[(y + 25) / GameWindow.tileSize + 1][(x + 25) / GameWindow.tileSize + 1].isFire){
            ...
            up = false;
            down = false;
            if(!dying) dyingStartTime = System.nanoTime();
            dying = true;
        }
    ...
    private void lineBomb (int x1, int y1){
        ...
        if (!GameState.grid[x1][y1].occupied){
            if (powerBomb>0) GameState.bombList.add(new Bomb(fuseLength, 15, this, (y1-1)*70, (x1-1)*70, canPierce));
            else GameState.bombList.add(new Bomb(fuseLength, bombRadius, this, (y1-1)*70, (x1-1)*70, canPierce));
        }
    ...
    public void draw(Graphics g){
        if(dying){
            if(System.nanoTime() - dyingStartTime  < 500000000) g.drawImage(death1, x, y - 35, null);
            else if(System.nanoTime() - dyingStartTime < 700000000) g.drawImage(death2, x, y - 35, null);
            else if(System.nanoTime() - dyingStartTime < 900000000) g.drawImage(death3, x, y - 35, null);
            else if(System.nanoTime() - dyingStartTime < 1100000000) g.drawImage(death4, x, y - 35, null);
            else if(System.nanoTime() - dyingStartTime < 1300000000) g.drawImage(death5, x, y - 35, null);
            else if(System.nanoTime() - dyingStartTime < 1800000000) g.drawImage(death6, x, y - 35, null);
        }
        else{
            if(right || left || up || down){
                if(direction.get(direction.size()-1) == 1){
                    if(Math.abs(startWalkingy - y) % 200 < 50 ) g.drawImage(up2, x, y - 35, null);
                    else if(Math.abs(startWalkingy - y) % 200 < 100 ) g.drawImage(up1, x, y - 35, null);
                    else if(Math.abs(startWalkingy - y) % 200 < 150 ) g.drawImage(up3, x, y - 35, null);
                    else if(Math.abs(startWalkingy - y) % 200 < 200 ) g.drawImage(up1, x, y - 35, null);
                }
                if(direction.get(direction.size()-1) == 2){
                    if(Math.abs(startWalkingx - x) % 200 < 50 ) g.drawImage(right2, x, y - 35, null);
                    else if(Math.abs(startWalkingx - x) % 200 < 100 ) g.drawImage(right1, x, y - 35, null);
                    else if(Math.abs(startWalkingx - x) % 200 < 150 ) g.drawImage(right3, x, y - 35, null);
                    else if(Math.abs(startWalkingx - x) % 200 < 200 ) g.drawImage(right1, x, y - 35, null);
                }
                if(direction.get(direction.size()-1) == 3){
                    if(Math.abs(startWalkingy - y) % 200 < 50 ) g.drawImage(down2, x, y - 35, null);
                    else if(Math.abs(startWalkingy - y) % 200 < 100 ) g.drawImage(down1, x, y - 35, null);
                    else if(Math.abs(startWalkingy - y) % 200 < 150 ) g.drawImage(down3, x, y - 35, null);
                    else if(Math.abs(startWalkingy - y) % 200 < 200 ) g.drawImage(down1, x, y - 35, null);
                }
                if(direction.get(direction.size()-1) == 4){
                    if(Math.abs(startWalkingx - x) % 200 < 50 ) g.drawImage(left2, x, y - 35, null);
                    else if(Math.abs(startWalkingx - x) % 200 < 100 ) g.drawImage(left1, x, y - 35, null);
                    else if(Math.abs(startWalkingx - x) % 200 < 150 ) g.drawImage(left3, x, y - 35, null);
                    else if(Math.abs(startWalkingx - x) % 200 < 200 ) g.drawImage(left1, x, y - 35, null);
                }
            }
            else{
    ...
    public void keyPressed(int k){
        if(!dying){
            ...
            if(k == KeyEvent.VK_Q&&type==1&&bombCount>0&&!GameState.grid[(y+25) / GameWindow.tileSize + 1][(x+25) / GameWindow.tileSize + 1].occupied){
                if(lineBomb>0){
                    ...
                    GameState.bombList.add(new Bomb(fuseLength, 15, this, ((x+25)/70)*70, ((y+25)/70)*70, canPierce));
                    powerBomb--;
                }
                else GameState.bombList.add(new Bomb(fuseLength, bombRadius, this, ((x+25)/70)*70, ((y+25)/70)*70, canPierce));
            }
