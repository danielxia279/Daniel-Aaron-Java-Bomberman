...
public class Tile{
    ...
    private static BufferedImage unbreakableSprite, breakableSprite,
            BombUpSprite,  bombKickSprite, fireSprite, lineBombSprite, powerBombSprite, bombPierceSprite, skateSprite,
            breaking1, breaking2, breaking3, breaking4, breaking5, breaking6,
            up_edge1, up_edge2, up_edge3, up_edge4, up_edge5,
            right_edge1, right_edge2, right_edge3, right_edge4, right_edge5,
            down_edge1, down_edge2, down_edge3, down_edge4, down_edge5,
            left_edge1, left_edge2, left_edge3, left_edge4, left_edge5,
            initial1, initial2, initial3, initial4, initial5,
            vertical1, vertical2, vertical3, vertical4, vertical5,
            horizontal1, horizontal2, horizontal3, horizontal4, horizontal5;
    ...
    /*
    1 = increase blast radius by 1
    2 = increase max bomb count by 1
    3 = move faster
    4 = First bomb's blast radius is entire map
    5 = can kick bombs
    6 = allows bomb to pierce
    7 = places bombs in a line in front of the player
     */
    public void power(Player player){
        if(powerup == 1) player.bombRadius++;
        else if(powerup == 2) player.bombCount++;
        else if(powerup == 3) player.speed++;
        else if(powerup == 4) player.powerBomb++;
        else if(powerup == 5) player.canKick = true;
        else if(powerup == 6) player.canPierce = true;
        else if(powerup == 7) player.lineBomb++;
    }
    ...
    public void draw(Graphics g){
        if(isFire){
            if(System.nanoTime() - FireStartTime < 50000000 || System.nanoTime() - FireStartTime >= 400000000){
                if(initial){
                    ...
                    if(edge) g.drawImage(up_edge1, x, y, null);
                    else g.drawImage(vertical1, x, y, null);
                }
                else if(down){
                    if(edge) g.drawImage(down_edge1, x, y, null);
                    else g.drawImage(vertical1, x, y, null);
                }
                else if(right){
                    if(edge) g.drawImage(right_edge1, x, y, null);
                    else g.drawImage(horizontal1, x, y, null);
                }
                else if(left){
                    if(edge) g.drawImage(left_edge1, x, y, null);
                    else g.drawImage(horizontal1, x, y, null);
                }
            }
            else if(System.nanoTime() - FireStartTime < 100000000 || System.nanoTime() - FireStartTime >= 350000000){
                if(initial){
                    g.drawImage(initial2, x, y, null);
                }
                else if(up){
                    if(edge) g.drawImage(up_edge2, x, y, null);
                    else g.drawImage(vertical2, x, y, null);
                }
                else if(down){
                    if(edge) g.drawImage(down_edge2, x, y, null);
                    else g.drawImage(vertical2, x, y, null);
                }
                else if(right){
                    if(edge) g.drawImage(right_edge2, x, y, null);
                    else g.drawImage(horizontal2, x, y, null);
                }
                else if(left){
                    if(edge) g.drawImage(left_edge2, x, y, null);
                    else g.drawImage(horizontal2, x, y, null);
                }
            }
            else if(System.nanoTime() - FireStartTime < 150000000 || System.nanoTime() - FireStartTime >= 300000000){
                if(initial){
                    g.drawImage(initial3, x, y, null);
                }
                else if(up){
                    if(edge) g.drawImage(up_edge3, x, y, null);
                    else g.drawImage(vertical3, x, y, null);
                }
                else if(down){
                    if(edge) g.drawImage(down_edge3, x, y, null);
                    else g.drawImage(vertical3, x, y, null);
                }
                else if(right){
                    if(edge) g.drawImage(right_edge3, x, y, null);
                    else g.drawImage(horizontal3, x, y, null);
                }
                else if(left){
                    if(edge) g.drawImage(left_edge3, x, y, null);
                    else g.drawImage(horizontal3, x, y, null);
                }
            }
            else if(System.nanoTime() - FireStartTime < 200000000 || System.nanoTime() - FireStartTime >= 250000000){
                if(initial){
                    g.drawImage(initial4, x, y, null);
                }
                else if(up){
                    if(edge) g.drawImage(up_edge4, x, y, null);
                    else g.drawImage(vertical4, x, y, null);
                }
                else if(down){
                    if(edge) g.drawImage(down_edge4, x, y, null);
                    else g.drawImage(vertical4, x, y, null);
                }
                else if(right){
                    if(edge) g.drawImage(right_edge4, x, y, null);
                    else g.drawImage(horizontal4, x, y, null);
                }
                else if(left){
                    if(edge) g.drawImage(left_edge4, x, y, null);
                    else g.drawImage(horizontal4, x, y, null);
                }
            }
                ...
                if(initial){
                    ...
                    if(edge) g.drawImage(up_edge5, x, y, null);
                    else g.drawImage(vertical5, x, y, null);
                }
                else if(down){
                    if(edge) g.drawImage(down_edge5, x, y, null);
                    else g.drawImage(vertical5, x, y, null);
                }
                else if(right){
                    if(edge) g.drawImage(right_edge5, x, y, null);
                    else g.drawImage(horizontal5, x, y, null);
                }
                else if(left){
                    if(edge) g.drawImage(left_edge5, x, y, null);
                    else g.drawImage(horizontal5, x, y, null);
                }
            }
        }
        ...
        if(type==4){
            ...
            if(System.nanoTime() - breakingStartTime < 75000000) g.drawImage(breaking1, x, y, null);
            else if(System.nanoTime() - breakingStartTime < 150000000) g.drawImage(breaking2, x, y, null);
            else if(System.nanoTime() - breakingStartTime < 225000000) g.drawImage(breaking3, x, y, null);
            else if(System.nanoTime() - breakingStartTime < 300000000) g.drawImage(breaking4, x, y, null);
            else if(System.nanoTime() - breakingStartTime < 375000000) g.drawImage(breaking5, x, y, null);
            else if(System.nanoTime() - breakingStartTime < 450000000) g.drawImage(breaking6, x, y, null);
        }
        else if (type==2){
