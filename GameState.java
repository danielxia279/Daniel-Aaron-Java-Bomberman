...
public class GameState extends StateTemplate {
    ...
    public static boolean checkBomb (int x, int y){
        ...
        y=y^x;
        x=x^y;
        int x1 = x/GameWindow.tileSize, y1 = y/GameWindow.tileSize, x2 = (x+69)/GameWindow.tileSize, y2 = (y+69)/GameWindow.tileSize;
        x1++; y1++; x2++; y2++;
        //y1++; y2++;
        //System.out.println(x+" "+y+" "+x1+" "+y1+" "+x2+" "+y2);
        //System.out.println(x+" "+y+" "+x1+" "+y1+" "+x2+" "+y2+" "+grid[x1][y1]+" "+grid[x2][y2]);
    ...
    private void blastUp(int distance, int x, int y, boolean canPierce){
        ...
        if(distance == 1 && !grid[x][y].isFire) grid[x][y].startFire(true);
        else grid[x][y].startFire(false);
    ...
    public static void playSound(){
        try{
            gameMusic = AudioSystem.getAudioInputStream(new File("assets/gameMusic.wav"));
            clip = AudioSystem.getClip();
            clip.open(gameMusic);
