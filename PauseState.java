...
public class PauseState extends StateTemplate {
    ...
    public void tick(){
        for(int i = 1; i <= 15; i++){
            for(int j = 1; j <= 15; j++){
                if(gameState.grid[i][j].isFire) gameState.grid[i][j].FireStartTime += frameTime;
                if(gameState.grid[i][j].isBreaking) gameState.grid[i][j].breakingStartTime += frameTime;
            }
        }
